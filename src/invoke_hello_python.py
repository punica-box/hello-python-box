...
class InvokeHelloPython(object):
    def __init__(self, sdk: OntologySdk, hex_contract_address: str):
        ...
        if len(hex_contract_address) != 40:
            raise RuntimeError('The length of contract address should be 40.')
        self.__sdk = sdk
    ...
    def echo(self, msg: str) -> str:
        ...
        echo = InvokeFunction('echo')
        echo.set_params_value(msg)
        response = self.__sdk.neo_vm().send_transaction(self.__contract_address_hex, None, None, 0, 0, echo, True)
    ...
    def notify_args(self, bool_args: bool, int_args: int, list_args: list, str_args: str, bytes_address_args: bytes,
        ...
        tx_hash = self.__sdk.neo_vm().send_transaction(self.__contract_address_hex, acct, payer_acct, gas_limit,
    ...
    def put_dict(self, dict_args: dict, acct: Account, payer_acct: Account, gas_limit: int, gas_price: int):
        put_dict = InvokeFunction('put_dict')
    ...
    def get_dict(self):
        get_dict = InvokeFunction('get_dict')
        response = self.__sdk.neo_vm().send_transaction(self.__contract_address_hex, None, None, 0, 0, get_dict,
                                                        True)
        return response

    def put_dict_value(self, dict_value_args, acct: Account, payer_acct: Account, gas_limit: int,
                       gas_price: int) -> str:
        put_dict_value = InvokeFunction('put_dict_value')
    ...
    def get_dict_value(self):
        get_dict_value = InvokeFunction('get_dict_value')
        response = self.__sdk.neo_vm().send_transaction(self.__contract_address_hex, None, None, 0, 0,
                                                        get_dict_value, True)
        return response

    def put_list(self, list_args: list, acct: Account, payer_acct: Account, gas_limit: int, gas_price: int):
        put_list = InvokeFunction('put_list')
    ...
    def get_list(self):
        get_list = InvokeFunction('get_list')
        response = self.__sdk.neo_vm().send_transaction(self.__contract_address_hex, None, None, 0, 0, get_list,
                                                        True)
        return response

    def add_key_value_in_dict(self, key, value, acct: Account, payer_acct: Account, gas_limit: int, gas_price: int):
        add_key_value_in_dict = InvokeFunction('add_key_value_in_dict')
    ...
    def query_add_key_value_in_dict_event(self, tx_hash):
        ...
        event[3] = {k.decode('ascii'): v.decode('ascii') for k, v in ContractDataParser.to_dict(event[3]).items()}
        return event

    def get_value_by_key(self, key, acct: Account, payer_acct: Account, gas_limit: int, gas_price: int):
        get_value_by_key = InvokeFunction('get_value_by_key')
        get_value_by_key.set_params_value(key)
        response = self.__sdk.neo_vm().send_transaction(self.__contract_address_hex, acct, payer_acct, gas_limit,
                                                        gas_price)
        return response
